...
def sig_info_augment(sig_info_dir):
	"""
	...
	Input:
		Mandatory:
		-sig_info_dir (str): The directory of sig_info file. E.g.,
		'./Data/sig_info.txt'
		
		
	Output:
		-sig_info_v1 (dataframe): A dataframe. It is like the input file,
		except it has four more columns.
	...
	print("Number of available gene expression signature: {}".format(sig_info.shape[0]))
	print("Number of available columns: {}".format(sig_info.shape[1]))

	
	dose=[x.split() if x!='-666' else ['-666', '-666']  for x in sig_info.pert_idose]
	time = [x.split() if x!='-666' else ['-666', '-666'] for x in sig_info.pert_itime]
	...
	adding = pd.DataFrame({'pert_dose': pert_dose, 'pert_dose_unit': pert_dose_unit, 
                      'pert_time':pert_time, 'pert_time_unit': pert_time_unit})
					  
	sig_info_v1= pd.concat([sig_info, adding], axis=1)
	print("Number of available columns after augmentation: {}".format(sig_info_v1.shape[1]))
